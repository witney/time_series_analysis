function [coh_data,phase_mean,phase_difference] = wc_get_phase_coherenceoutput_phases(data, s_rate, frequency_range)
...
%     phase_difference=angle(exp(1i * (data1 - data2)));
    phase_difference = [phase_difference; channel_n1 channel_n2 angle(exp(1i * (data1 - data2)))];
%     phase_mean(channel_n1,channel_n2)=circ_mean(phase_difference);
  end
end

phase_mean=[];
% phase_mean=circ_mean(phase_difference);

% phase_mean=[];
% for g=1:size(phase_difference,1)
%     phase_mean(g) = circ_mean(phase_difference(g,3:end));
% end
